...
def extract_colors(colors=[]):
    ...
    for color in colors:
        [color_val] = [(c, v)
                       for c, v in pygame.color.THECOLORS.items() if color == c]
...
robot_colors = extract_colors(
    ['blue', 'orange', 'yellow', 'green', 'red', 'purple', 'violet', 'magenta', 'blue', 'orange'])
...
class Environment:
    ...
    def draw_env(self, FS):
        ...
        self.surface = pygame.display.set_mode(
            (self.map_width*self.scaling_factor, self.map_height*self.scaling_factor))
        ...
        for i in range(self.environment.shape[0]):
            for j in range(self.environment.shape[1]):
                ...
                pygame.draw.rect(self.surface, color, pygame.Rect(
                    j*self.scaling_factor, (i)*self.scaling_factor, self.scaling_factor, self.scaling_factor))
        pygame.display.update()


FS = fire_swarm.Fire_Swarm(25, [50, 50], 10)
env = Environment(FS.simulated_space)
running = True

while running and FS.time_steps < 1000:
